...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        credits_context = [{"id": 6, "credit": 1, "subject_id_list_that_using_this_credit": [3]},#Chemistry
                           {"id": 7, "credit": 2, "subject_id_list_that_using_this_credit": [2, 4]},#Physics Algorithm
                           {"id": 9, "credit": 3, "subject_id_list_that_using_this_credit": [1, 5]}]#Math Coding
        ...
        def mapCredit(title):
            subject_id = [subject for subject in subjects_context if subject['title'] == title][0]['id']
            newCredit = [credit for credit in credits_context if subject_id in credit['subject_id_list_that_using_this_credit']][0]['credit']
            return newCredit,subject_id
        ...
        #get studentID and subjectID
        try:
            studentID = Personnel.objects.get(first_name= student_first_name,last_name=student_last_name).pk
            

            
        except:
            return Response(" '{} {}' not found in Database".format(student_first_name, student_last_name),status=status.HTTP_400_BAD_REQUEST)

        try:
            subjectCredits,subjectID = mapCredit(subjects_title)
        except:
            return Response(" '{}' not found in Database".format(subjects_title,),status=status.HTTP_400_BAD_REQUEST)

        

             
        findStudent = StudentSubjectsScore.objects.filter(student_id=studentID,subjects_id=subjectID)
        
        
        if  findStudent.count()>0:
            findStudent.update(score=score)
                
        else:
            
            new_data = StudentSubjectsScore.objects.create(student_id=studentID,subjects_id=subjectID,score=score,credit=subjectCredits)
            new_data.save()


            #return student's details, subject's title, credit and score
        return Response("Student's name: {} {} , subject's title: {} , credit: {} , score: {}".format(student_first_name,student_last_name,subjects_title,subjectCredits,score),status=status.HTTP_201_CREATED)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        def calculateGrade(studentScore):
            if studentScore >= 80:
                return "A",4
            elif studentScore >= 75:
                return "B+",3.5
            elif studentScore >= 70:
                return "B",3
            elif studentScore >= 65:
                return "C+",2.5
            elif studentScore >= 60:
                return "C",2
            elif studentScore >= 55:
                return "D+",1
        ...
        def createSubjectDetailsAndGPA(subjectList):
            subjectDetails = []
            sumCredit = 0
            sumGradePoint = 0
            for data in subjectList:
                sumCredit += data.credit
                sumGradePoint += calculateGrade(data.score)[1]*data.credit
                subjectDetails.append({
                "subject":data.subjects.title,
                "credit":data.credit,
                "score":data.score,
               "grade":calculateGrade(data.score)[0]
               })
            return subjectDetails,"{:.2f}".format(sumGradePoint/sumCredit)
        ...
        #get fullname
        student_first_name = Personnel.objects.get(id=student_id).first_name
        student_last_name = Personnel.objects.get(id=student_id).last_name
        full_name = "{} {}".format(student_first_name,student_last_name)

        #get school name
        school_name= Personnel.objects.get(id=student_id).school_class.school

        #get subject detail
        subject_detail,gpa = createSubjectDetailsAndGPA(StudentSubjectsScore.objects.filter(student=student_id).all())
        
            


        response_detail = {"student":{"id":student_id,"full_name":full_name,"school":str(school_name)},"subject_detail":subject_detail,"grade_point_average":gpa}
      
        
        
        
        #school_name = Schools.objects.get(id = school_id).title
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        def getResult(PersonnelList,school_title):
            your_result = []
            for index,personel in enumerate(PersonnelList):
            
                #get full name
                fullname = "{} {}".format(personel['first_name'] ,personel['last_name'])

                #get role
                choice = Personnel.objects.get(id = personel['id']).personnel_type
                role = Personnel.personnel_type.field.choices[choice][0]
                class_order = Classes.objects.get(id =personel['school_class_id']).class_order


                your_result.append("{}. school: {}, role: {}, class: {},name: {}".format(index+1,school_title,role,class_order,fullname))
        ...
        #get school class id
        school_id = Schools.objects.get(title=school_title).pk
        school_class =Classes.objects.filter(school=school_id).values_list('id', flat=True)
        PersonnelList = Personnel.objects.filter(school_class_id__in=school_class).values().order_by('personnel_type',"school_class_id",'first_name')
        ...
        return Response(getResult(PersonnelList,school_title), status=status.HTTP_200_OK)
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        #get school id order by title
        schoolObject = Schools.objects.all().order_by("title").values()

        #get class order id for class order and class id for key in personnel
        classObject = Classes.objects.all().values()

        #get name and role
        role = Personnel.personnel_type.field.choices[0][0]
        personnelObject = Personnel.objects.all().values()

        for school in schoolObject:
            schoolDict = {}
            schoolName = school['title']
            schoolDict["school"] = schoolName
            for classes in Classes.objects.filter(school_id = school['id']):
                classesOrder = classes.class_order

                studentList = []
                teacher = ""

                for personnel in Personnel.objects.filter(school_class_id = classes.pk).order_by('personnel_type','first_name'):
                    
                    role = Personnel.personnel_type.field.choices[personnel.personnel_type][0]
                    if personnel.personnel_type == 0:
                        teacher = "{}: {}".format(role,personnel.first_name+personnel.last_name)
                    else:
                        studentName = "{} {}".format(personnel.first_name,personnel.last_name)
                        studentList.append({role:studentName})

                class_value = {teacher:studentList}
                schoolDict["class {}".format(classesOrder)] = {teacher:studentList}

            your_result.append(schoolDict)

            #{school:xxxx,class1:xxxx,class2:xxxx,class3:xxxx,class4:xxx,class5:xxx}
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        highSchoolLevel = SchoolStructure.objects.filter(parent=None).values()

        
        for level in highSchoolLevel:
            subHighSchoolDict = {}
            levelList = []
            
            
            for grade in SchoolStructure.objects.filter(parent=level['id']).values():
                
                levelDict = {}
                subGrade = []
                for room in SchoolStructure.objects.filter(parent = grade['id']).values():
                    
                    subGrade.append({"title":room['title']})
                    levelDict["title"] = grade['title']
                    levelDict['sub'] = subGrade
                levelList.append(levelDict)

                
                subHighSchoolDict["title"] = level['title']
                subHighSchoolDict['sub'] = levelList
                #print(levelDict)
                print(subHighSchoolDict)
            print("__________________")
            your_result.append(subHighSchoolDict)
